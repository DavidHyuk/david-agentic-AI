...
def test_watchdog_checks_frequently_and_recovers_stale_locks():
    ...
    assert "--hermes-home %h/.hermes/profiles/english-podcast" in service
    assert "--gateway-service hermes-gateway-english-podcast.service" in service
    assert "--skip-missing-gateway" in service
    assert "--skip-missing-home" in service
    assert "TimeoutStartSec=270" in service

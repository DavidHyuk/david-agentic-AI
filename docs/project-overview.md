...
- 전문 에이전트는 평소 역할별 워크스테이션에 머물고, 팀장 Hermes만 주기적으로
  팀원을 찾아가 가벼운 점검 대화를 합니다. 따라서 이동은 장식적 로밍이 아니라
  ...
  말풍선은 약 7–14초의 휴지 간격으로 각
  방의 실제 최신 cron 응답을 한 줄로 줄인 `RECENT NOTICE`, 최근 응답이 없는
  경우의 다음 알림 일정, 역할별 일상 대사, 가까이 있는 두 캐릭터의 `TEAM CHAT`을 순환해
  표시하며 선택 시 즉시 인사합니다. 알림 발췌는 저장된 데이터이고 팀 대화는
  실제 업무로 오인하지 않도록 표시한 연출입니다. 수동 말풍선은 모델을 호출하거나
  음성을 재생하지 않으며, 숨겨진 화면과 기록 리플레이에서는 중단됩니다.
  캐릭터 위에 마우스를 올리면 진행 중인 팀 대화를 방해하지 않는 범위에서 다음
  알림 일정(일정이 없으면 최근 알림)을 즉시 말풍선으로 보여줍니다.
  한 장면은 네 번의 짧은 교대로 마무리되어 말이 끊겨 보이지 않습니다. 캐릭터
  호버에는 최근 기록과 David의 다음 행동을 함께 보여줍니다. 첫 연출은 가까운
  연구·면접 캐릭터의 대화로 시작하며, 모바일에서는 `−/＋` 버튼과 두 손가락
  핀치로 사무실·가구·캐릭터 전체를 60–140% 확대/축소합니다. 모바일 최소
  배율에서도 워크스테이션 간격을 유지하고 상단 캐릭터 및 English Lounge 명패에
  별도 오프셋을 적용해 캐릭터·이름 카드·방 이름이 겹치지 않습니다.

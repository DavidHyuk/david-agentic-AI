...
- 일곱 캐릭터는 책상·커피 공간·소파·창가 사이의 통로를 실제로 이동합니다.
  무작위 목적지를 고르는 대신 연구 자료실·화이트보드·협업 테이블·코칭
  라운지·오디오 데스크 등 직무별 고정 루틴을 따르고, 이름 아래 현재 목적을
  표시합니다. 사무실은 리서치/시스템 보드, 자료 선반, 리뷰 보드와 구역 러그로
  업무 영역을 구분합니다. 각 장소에서 약 75–140초 머무르고 출발 시점도
  분산합니다. 캐릭터 이름과 현재 목적은 항상 읽을 수 있는 상태 카드로 표시합니다.

...
arXiv / Hugging Face
        │
        └── papers_ingest.py ── SQLite ── daily systemd timer
...
- 전문 에이전트는 평소 역할별 워크스테이션에 머물고, 팀장 Hermes는 오피스를
  연 뒤 수초 내에 섞인 방문 deck에서 무작위 팀원을 찾아가 점검 대화를 합니다.
  한 deck 안에서는 전 팀원을 한 번씩 방문하며, 상대별 여러 대사 중 직전 대사를
  제외하고 골라 같은 상대와 같은 문장이 짧은 시간 안에 반복되지 않습니다.
  따라서 이동은 반복되는 고정 경로가 아니라
  리더십 방문이라는 읽을 수 있는 상태 변화입니다. 사무실은 담당자 이름을 먼저
  보여주는 `IRIS · THEO / RESEARCH · INTERVIEW`와
  `JUN · MINA / CODE · SYSTEM DESIGN` 벽면 명패, Espresso Coffee Club과
  분리된 Ellie English Lounge, Rina가 실제 안에 서는 흡음 패널형 ON AIR 부스,
  협업 테이블과 자료·리뷰 보드로 업무 영역을 구분합니다. 명패는 캐릭터 레이어와
  분리되어 데스크톱·모바일 축소 화면에서도 캐릭터가 가리지 않습니다. 각 방에는 실제
  저장 상태에서 계산한 사용자의 다음 행동(읽기 대기 논문, 미완료 과제, SRS 복습
  수 등)을 보여주는 클릭 가능한 카드가 있어 해당 작업실로 바로 이동합니다.
  캐릭터 이름과 현재 목적은 항상 읽을 수 있는 상태 카드로 표시합니다.
  말풍선은 약 3.2–6초의 짧고 무작위인 휴지 간격으로 각 방의 실제
  `David 다음 행동`을 우선 표시하고, 역할별 일상 대사와 무작위 두 캐릭터의
  `TEAM CHAT`을 섞습니다. 같은 방·대화 장면은 연속 반복하지 않으며 선택 시
  즉시 인사합니다. 다음 행동은 저장된 데이터이고 팀 대화는
  실제 업무로 오인하지 않도록 표시한 연출입니다. 수동 말풍선은 모델을 호출하거나
  음성을 재생하지 않으며, 숨겨진 화면과 기록 리플레이에서는 중단됩니다.
  캐릭터 위에 마우스를 올리면 진행 중인 팀 대화를 방해하지 않는 범위에서 최근
  기록과 David의 다음 행동을 즉시 보여줍니다. Hermes의 팀원 방문과 팀원끼리의
  대화는 모두 네 문장을 번갈아 말하는 2회 왕복으로 진행하며, 답변 사이에 읽을
  시간을 두고 마지막 답변까지 유지합니다. 대화 중간에는 별개의 다음 행동 알림으로
  말풍선을 덮어쓰지 않습니다. 무대는 실제 컨테이너 폭을 관찰해 780px
  기준 장면을 자동 축소하므로 MacBook 전체 화면과 분할 창 모두 사무실 전체를 먼저
  보여줍니다. 사용자가 확대했을 때만 좌우 스크롤이 생깁니다. 모바일에서는 `−/＋`
  버튼과 두 손가락 핀치로 사무실·가구·캐릭터 전체를 60–140% 확대/축소합니다.
  100% 미만으로 축소해도 무대 배경 폭은 컨테이너 끝까지 채우고 고정 크기 캐릭터와
  라벨만 상대적으로 작아지므로 오른쪽에 빈 띠가 생기지 않습니다.
  축소 화면에서도 워크스테이션 간격을 유지하고 상단 캐릭터 및 English Lounge 명패에는
  별도 오프셋을 적용합니다. 실제 오피스 컨테이너 폭 900px 이하에서는 compact
  mode를 적용합니다.
  이 모드는 캐릭터 위 업무 설명을 숨기고 작은 이름 pill만 남기되, 말풍선은 크기와
  여백만 줄여 말하는 캐릭터 머리 위에 직접 표시합니다. 각 말풍선은 live status로
  보조 기술에도 전달됩니다. 다음 행동 카드는 2열로 넓혀 Galaxy Fold 가로 화면에서도
  캐릭터와 텍스트가 서로 가리는 영역을 줄입니다.
  Hermes가 가까운 팀원에게 말을 걸 때 두 말풍선은 좌우 바깥쪽으로 이동하며, 네 번째
  답변은 약 4초 동안 유지되고 중간에 다음 행동 문구로 덮어쓰지 않습니다.
  데스크톱과 노트북에서는 사이드바 가장자리의 `‹`를 눌러 왼쪽으로 접고 오피스를
  전체 창 너비로 확장할 수 있으며 `›`로 원복합니다. 선택은 브라우저에 저장되고,
  모바일 상단 내비게이션은 저장값과 관계없이 항상 유지됩니다.
...
- 캐릭터 클릭은 사무실 위 초상화·대화 패널을 열며 작업실에서도 같은 대화를
  이어갑니다. 작업실 동반 캐릭터는 잘림 없는 전신 비율로 표시되고, 작업실 경로는
  브라우저 히스토리에 기록되어 Back으로 이전 Observatory 화면에 돌아갑니다. 웹 전용 대화는 기존 HQ API의 `office_<room>` 세션에 보존하고
  ...
  대화 세션 준비 단계의 gateway 응답은 15초로 제한해 고장 시 화면에 명확한
...
- **MCP (Model Context Protocol)**: 외부 서비스를 표준 도구로 연결. Google
  Calendar 템플릿은 추후 재활성화를 위해 보존됩니다.
...
- 오피스 중앙 HUD에는 잔액·연속 달성·다음 가상 오퍼 진행률이 표시됩니다. HUD를
  클릭하면 다음 카드까지 남은 CASH, 활동별 획득액·주간 진척·콤보/주간 보너스와
  해당 작업실 바로가기를 확인할 수 있습니다.

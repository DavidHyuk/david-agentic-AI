...
§
Local LLM: Qwen3.5-122B-A10B-AWQ via vLLM at http://localhost:8003/v1 (run local-model/run_hermes_model.sh). Same weights as ClawGram. Sampling: presence_penalty=0.6, repetition_penalty=1.05 via Hermes providers.qwen-hermes.extra_body (not vllm CLI). Subscribe-Papers still uses llama.cpp gpt-oss-120b on :8080.
§

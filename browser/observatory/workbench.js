...
function coachDesk(d) {
  ...
  if (!a)
    return lcSummary + `<article class="desk-card"><span class="tag">TODAY</span><h2>${d.today_assignment?.completed ? "오늘 과제를 완료했습니다" : "이어갈 미완료 과제가 없습니다"}</h2><p>${d.today_assignment?.completed ? "한 문제 더 풀고 싶다면 학습 기록에 맞는 다음 과제를 배정할 수 있습니다." : "학습 기록에 맞춰 오늘의 과제를 배정합니다."} 과제 배정은 학습 완료로 기록되지 않습니다.</p><button id="desk-plan" class="primary" ${!d.catalog_available ? "disabled" : ""}>${d.today_assignment?.completed ? "다음 과제 준비하기" : "오늘 과제 준비하기"}</button>${!d.catalog_available ? "<p>커리큘럼 카탈로그가 아직 설치되지 않았습니다.</p>" : ""}</article>`;
  const item = a.item,
